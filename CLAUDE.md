...
Same logic lives in `src/generators/chassis.ts` for the in-browser Build tab.
Bumper arc: circle center (25, 0) mm, R = 85 mm. X-flipped at generation so bumper faces −X.

### Earcut hole constraint — IMPORTANT for generator work

Every hole pushed to `s.holes` must be **entirely inside** the outer shape boundary at every y value the hole occupies. Two zones:
...
If any hole point is outside its zone's boundary, Three.js earcut creates a large phantom triangle spanning the whole face. The symptom is a light-gray diagonal wedge covering most of the chassis in the viewer. Fix: move the hole into a zone wide enough to contain it, or reduce its x extent.
