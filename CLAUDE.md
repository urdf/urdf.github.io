...
## Resizable panel — UX best practices

Sources: W3C APG Window Splitter pattern · VS Code sash · Grafana/Figma precedent

| Decision | Rule |
|----------|------|
| Handle | 8 px hit target; 1 px `::after` visual line; `col-resize` cursor (semantically correct for vertical splitter — not `ew-resize`) |
| Hover affordance | Line widens to 2 px, tinted `--blue`; no grip icon — cursor change is sufficient |
| Drag transition | `transition: none` on `<main>` during drag to prevent right-offset lag; restore on `pointerup` |
| Snap magnet | ±10 px zone around default width snaps to exactly 380 px |
| Double-click | Resets to default (380 px); no animation needed |
| Keyboard | `role="separator"` + `tabindex="0"`; ← widen 16 px · → narrow 16 px · `Home` = min · `End` = max; update `aria-valuenow` on every change |
| Tooltip | Width in px shown near cursor while dragging only; reuses `#part-label` element |
| Limits | Min 260 px · max min(700 px, 60 vw) |
| Persistence | `localStorage` key `urdf-panel-w`; save on `pointerup`; **do not** persist open/closed state |
| Window resize | Clamp to `maxW()` with 50 ms debounce; do not overwrite stored value — clamp is temporary |
| Mobile | Handle hidden (`max-width: 768px`); stored width not restored on mobile |
